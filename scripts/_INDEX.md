...
| File | Responsibility |
|------|----------------|
| `build-extension.mjs` | Copies `src` → `dist/extension`, validates manifest references, excludes reference-only modules (classifier, feature-policy, site-overrides). No bundler — content script ships as-is |
